...
// Generate UPC: SUR-{CLOTH_PREFIX}-{YEAR}{MONTH}-{SEQ}
...
// POST /api/bolts/batch — add multiple bolts from one seller
router.post("/batch", requireAuth, async (req,res) => {
  try {
    ...
    for (const bolt of bolts) {
      ...
      // Default selling price = cost + 90% of cost (i.e. cost * 1.9)
      const markup = parseFloat(bolt.markup_pct) ?? 90; // Default: Cost + 90% = Cost × 1.9
      ...
      await db("bolts").insert({
        ...
        fabric_category: bolt.fabric_category||bolt.cloth_type,
...
router.get("/recommendations", requireAuth, async (req,res) => {
  try {
    // Find bolts where >60% sold within 30 days
...
router.post("/", requireAuth, async (req,res) => {
  try {
    const {cloth_type,fabric_category,description,total_meters,cost_price_per_meter,selling_price_per_meter,markup_pct,color,seller_name,seller_phone,notes,received_date} = req.body;
    ...
    const markup = parseFloat(markup_pct) ?? 90; // Cost + 90% of cost
    const sellingPrice = selling_price_per_meter ? parseFloat(selling_price_per_meter) : Math.round(costPrice*(1+markup/100));
    await db("bolts").insert({
      id:upc, cloth_type, fabric_category:fabric_category||cloth_type,
...
router.patch("/:id", requireAuth, async (req,res) => {
  try {
    const allowed = ["selling_price_per_meter","cost_price_per_meter","markup_pct","color","seller_name","status","remaining_meters","description"];
    const updates = {};
